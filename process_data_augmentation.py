...
def StoreData(directory,subdir):
    ...
    if(if_augment==1):
        for i in range(3):
            dir_l1 = sorted(os.listdir(directory+subdir[i*2]))
            dir_l2 = sorted(os.listdir(directory+subdir[i*2+1]))
            print(f"{subdir[i*2][:-1]} : {len(dir_l1)}")
            print()

            l=[]
            for j in range(len(dir_l1)):
                l.append(plt.imread(directory+subdir[i*2]+dir_l1[j]))

            l2=[]
            for j in range(len(dir_l2)):
                l2.append(plt.imread(directory+subdir[i*2+1]+dir_l2[j]))

            X_train, X_test, y_train, y_test = train_test_split(l, l2, train_size=train_test_split_)

            X_aug=[] ; y_aug=[] ;

            for j in range(400-len(X_train)): #400-100
                if j<len(X_train): #100
                    if(augmentation_type==0):
                        image,mask = light_augmentation(X_train[j],y_train[j])
                    elif(augmentation_type==1):
                        image,mask = medium_augmentation(X_train[j],y_train[j])
                    else:
                    ...
                    while(k>len(X_train)):
                        k-=len(X_train)
                    if(augmentation_type==0):
                        image,mask = light_augmentation(X_train[k-len(X_train)],y_train[k-len(X_train)])
                    elif(augmentation_type==1):
                        image,mask = medium_augmentation(X_train[k-len(X_train)],y_train[k-len(X_train)])
                    else:
